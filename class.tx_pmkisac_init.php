...
class tx_pmkisac_init {
	...
	function setup($conf) {
		...
		if (defined('T3MOOTOOLS')) {
			...
			$headerData .= '<script type="text/javascript" src="' . t3lib_extMgm::siteRelPath($this->extKey) . 'res/mootools_v1.2.1.js"></script>';
		}
		
		// Include shared JS
		$headerData .= '<script type="text/javascript" src="' . t3lib_extMgm::siteRelPath($this->extKey) . 'res/Autocompleter.js"></script>';
...
window.addEvent("domready", function(){
	...
	if (el) {
		...
		var completer = new Autocompleter.Request.HTML(el, ajaxurl , {
			...
			.'"delay": 100,
			'.($progressIndicator ? '"indicatorClass": "autocompleter-loading",':'').'
